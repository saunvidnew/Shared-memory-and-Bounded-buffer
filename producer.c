...
// PPass this to ftruncate and mmap
...
int main(int argc, char* argv[])
{
	...
	if(0 > bufSize || bufSize > 1000){
		printf("Buffer size must be within 0 and 1000");
		exit(1);
...
void InitShm(int bufSize, int itemCnt)
{
    int in = 0;
    int out = 0;
    const char *name = "OS_HW1_DianaMonreal"; // Name of shared memory object to be passed to shm_open
    int shm_fd; //Create a file descriptor
  ...
  // **Extremely Important: map the shared memory block for both reading and writing
    ...
    shm_fd = shm_open(name, O_CREAT | O_RDWR, 0666); //Create a shared memory object.
    ftruncate(shm_fd, SHM_SIZE); //Set the size of the file.
    gShmPtr = mmap(0, SHM_SIZE, PROT_READ | PROT_WRITE,  MAP_SHARED, shm_fd, 0); //Map the file to the memory.
...
void Producer(int bufSize, int itemCnt, int randSeed)
{
	...
	int val;
	for( i = 0; i < itemCnt; i++){
		while(((in + 1) % bufSize) == GetOut()); //Do nothing.
		val = GetRand(0, 3000);
...
// Get the value of the shared variable "itemCnt"
int GetItemCnt()
{
	return GetHeaderVal(1);
}

// Get the value of the shared variable "in"
int GetIn()
{
	return GetHeaderVal(2);
}

// Get the value of the shared variable "out"
int GetOut()
{
